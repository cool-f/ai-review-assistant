...
def combine_scores(keyword: float, semantic: float | None) -> tuple[float, str]:
    if semantic is None:
        return round(keyword, 4), "keyword"
    if keyword <= 0:
        return round(semantic, 4), "vector"
    return round(keyword * 0.55 + semantic * 0.45, 4), "hybrid"
...
class KPMatcher:
    ...
    async def match(self, solution_text: str, course_id: str) -> list[dict]:
        """
        将解答文本与所有知识点进行关键词匹配
        ...
        result = await self.db.execute(
            ...
            .where(Courseware.course_id == course_id)
        ...
        try:
            ...
            try:
                query_embedding = await service.embed_single(solution_text, text_type="query")
            finally:
                await service.close()
            distance = KnowledgePoint.embedding.cosine_distance(query_embedding)
            semantic_result = await self.db.execute(
                select(KnowledgePoint.id, distance.label("distance"))
                .join(Courseware, Courseware.id == KnowledgePoint.courseware_id)
                .where(
            ...
            semantic_scores = {
                row.id: max(0.0, min(1.0, 1.0 - float(row.distance)))
                for row in semantic_result
            }
        except Exception:
            logger.warning("向量匹配不可用，回退到关键词匹配", exc_info=True)

        scored: list[dict] = []
        kp_by_id = {kp.id: kp for kp in all_kps}
        for kp_id in set(keyword_scores) | set(semantic_scores):
            score, method = combine_scores(keyword_scores.get(kp_id, 0.0), semantic_scores.get(kp_id))
            if score >= 0.3:
                scored.append({
                    "knowledge_point_id": kp_id,
                    "title": kp_by_id[kp_id].title,
