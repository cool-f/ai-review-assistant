...
from review_assistant.infrastructure.persistence.models import Course, Courseware
from review_assistant.interfaces.http.schemas.courseware import (
    CoursewareResponse,
    CoursewareListResponse,
)
from review_assistant.application.ingestion.pipeline import run_extraction_pipeline
...
async def delete_courseware(
    ...
    if courseware.status == "processing":
        raise HTTPException(status_code=409, detail="课件正在处理，暂不能删除")
