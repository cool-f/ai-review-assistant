...
from review_assistant.infrastructure.persistence.models import Course, Homework, Solution, SolutionKnowledgePoint
from review_assistant.interfaces.http.schemas.homework import (
    HomeworkResponse,
...
async def preflight_homework(file: UploadFile = File(...)):
    ...
    try:
        return await preflight_document(
...
async def upload_homework(
    ...
    homework = Homework(
        ...
        file_hash=file_hash,
        status="ready",  # 题目已提取，等待用户明确触发 AI 解答
    )
    db.add(homework)
...
async def list_homeworks(
    ...
    # 构建查询条件
    conditions = [Homework.course_id == course_id]
...
async def get_homework(
    ...
    result = await db.execute(
        ...
        .where(Homework.id == homework_id, Homework.course_id == course_id)
    )
...
async def delete_homework(
    ...
    """
    result = await db.execute(
        select(Homework).where(Homework.id == homework_id, Homework.course_id == course_id)
    )
    homework = result.scalar_one_or_none()
    ...
    if homework.status == "processing":
        raise HTTPException(status_code=409, detail="作业正在解答，暂不能删除")
...
async def solve_homework(
    ...
    """
    existing_job = await get_homework_job(homework_id)
    if existing_job is not None:
        current_homework = await db.scalar(
            select(Homework).where(
                Homework.id == homework_id,
                Homework.course_id == course_id,
            )
        )
        if current_homework is None:
            raise HTTPException(status_code=404, detail="作业不存在")
        if should_replay_homework_job(existing_job, current_homework.status):
            return StreamingResponse(
                existing_job.subscribe(),
        ...
        await discard_terminal_homework_job(homework_id, existing_job)

    # 原子领取任务，避免并发请求重复调用模型并重复计费。
    ...
    await db.commit()
    # Start before returning StreamingResponse. Client disconnects only detach a
    # subscriber; they cannot leave the already-claimed homework idle forever.
    job = await start_homework_job(homework_id)
