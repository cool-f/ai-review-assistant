...
import { useSSE } from "@/shared/sse/useSSE";
import {
  listSessions,
  createSession,
  getHistory,
...
export default function ChatPanel({
  ...
  const { start, stop, isStreaming, isReconnecting, error: sseError } =
    useSSE({
      ...
      onReplace: (content: string) => {
        setStreamingTokens([content]);
        setMessages((prev) => prev.map((message) =>
          message.id === streamingMsgIdRef.current ? { ...message, content } : message,
        ));
      },
      onDone: (data: { message_id: string; token_count: number; citations?: unknown[] }) => {
  ...
  const loadSessions = useCallback(async () => {
    ...
    try {
      const data = await listSessions(courseId, 1, 50);
  ...
  const loadMessages = useCallback(async (sessionId: string) => {
    try {
      const data = await getHistory(sessionId, courseId);
  ...
  const handleSend = async () => {
    ...
    // 发起 SSE 流式请求
    start(`/api/chat/sessions/${activeId}/messages?course_id=${encodeURIComponent(courseId)}`, {
      content: text,
