...
import type { Folder, Courseware, Homework, KnowledgePoint, PaginatedResponse, StudyProgress, CoursewareProgressSummary } from "@/shared/types";
...
function StatusBadge({ status }: { status: string }) {
  ...
  switch (status) {
    ...
    case "ready":
      return <span className={`${base} bg-blue-100 text-blue-700`}>待解答</span>;
    case "failed":
      return <span className={`${base} bg-red-100 text-red-700`}>失败</span>;
    case "partial":
      return <span className={`${base} bg-orange-100 text-orange-700`}>部分完成</span>;
    default:
...
export default function FolderTree({ courseId, category, onSelect, progressRefreshKey = 0, dataRefreshKey = 0 }: FolderTreeProps) {
    ...
    try {
      ...
      const [folderRes, itemRes] = await Promise.all([
        client.get<{ items: Folder[]; total: number }>("/folders/", {
          params: { category, course_id: courseId },
        }),
        client.get<PaginatedResponse<ItemNode>>(
          category === "courseware" ? "/coursewares/" : "/homeworks/",
          { params: { course_id: courseId, folder_id: "null", size: 100 } },
        ),
      ]);
  ...
  useEffect(() => {
    ...
    if (!allItems.some((item) => item.data.status === "processing")) return;
    const timer = window.setInterval(() => void loadData(), 2500);
    return () => window.clearInterval(timer);
  }, [state, folderChildren, loadData]);
  ...
  }, [courseId, progressRefreshKey, expandedCoursewares]);
  ...
  const toggleFolder = useCallback(
      ...
      if (!folderChildren.has(folderId) && !expandedFolders.has(folderId)) {
        ...
        try {
          const res = await client.get<PaginatedResponse<ItemNode>>(
            category === "courseware" ? "/coursewares/" : "/homeworks/",
            { params: { course_id: courseId, folder_id: folderId, size: 100 } },
          );
    ...
    [category, courseId, expandedFolders, folderChildren],
  );
  ...
  const toggleCourseware = useCallback(
        ...
        try {
          const [kpRes, progressRes] = await Promise.all([
            client.get<PaginatedResponse<KnowledgePoint>>(
              ...
              { params: { course_id: courseId, size: 100 } },
            ),
    ...
    [courseId, expandedCoursewares, coursewareChildren],
  );
  ...
  const handleCreateFolder = useCallback(async () => {
    ...
    try {
      await client.post("/folders/", { name, category, course_id: courseId });
      setNewFolderName("");
  ...
  }, [newFolderName, category, courseId, loadData]);
  ...
  const handleRenameFolder = useCallback(
    async (folderId: string) => {
      ...
      try {
        await client.patch(`/folders/${folderId}`, { name }, { params: { course_id: courseId } });
        setRenamingFolderId(null);
        await loadData();
      } catch {
  ...
  const handleDeleteFolder = useCallback(
    (folder: Folder) => {
      setConfirmConfig({
        ...
        onConfirm: async () => {
          try {
            await client.delete(`/folders/${folder.id}`, { params: { course_id: courseId } });
  ...
  const handleDeleteItem = useCallback(
    ...
    [category, courseId, loadData],
  );

  const handleRetryCourseware = useCallback(async (coursewareId: string) => {
    try {
      await client.post(
        `/coursewares/${coursewareId}/knowledge-points/extract`,
        {},
        { params: { course_id: courseId } },
      );
    } catch (error) {
      ...
      if (status !== 409) {
        console.error("课件重试失败", error);
        return;
      }
      const confirmed = window.confirm(
        "该操作需要全量重提取，并会删除此课件已有的知识点、练习和进度。是否继续？",
      );
      if (!confirmed) return;
      await client.post(
        `/coursewares/${coursewareId}/knowledge-points/extract`,
  ...
  const renderItemNode = useCallback(
    (item: ItemTreeNode, depth: number = 0) => {
      ...
      const courseware = isCourseware ? item.data as Courseware : null;
      ...
      return (
        <li key={item.id}>
          <div
            ...
            className="group flex cursor-pointer items-center gap-1.5 rounded-md px-2 py-2 text-sm text-gray-700 transition-colors hover:bg-gray-100"
            style={{ paddingLeft: `${12 + depth * 16}px` }}
            title={courseware?.error_message ? `${item.label}：${courseware.error_message}` : item.label}
          >
            {/* 展开箭头（课件）或叶子图标 */}
            <span className="flex w-4 shrink-0 items-center justify-center">
              {isCourseware ? (
            ...
            {courseware?.embedding_status === "failed" && (
              <span
                className="shrink-0 rounded-full bg-amber-100 px-1.5 py-px text-[10px] font-medium text-amber-800"
                title="向量索引不可用；浏览仍可用，但语义检索、引用匹配和关联发现可能不完整"
              >
                语义能力降级
              </span>
            )}

            {courseware && (courseware.status === "failed" || courseware.status === "partial") && (
              <button
                type="button"
                onClick={(event) => {
                  event.stopPropagation();
                  void handleRetryCourseware(item.id);
                }}
                className="shrink-0 rounded px-1 text-[10px] text-blue-600 hover:bg-blue-50"
                title={`失败阶段：${courseware.failed_stage ?? "未知"}，点击重试`}
              >
                重试
              </button>
            )}
    ...
    [onSelect, handleDeleteItem, handleRetryCourseware, expandedCoursewares, loadingCourseware, coursewareChildren, toggleCourseware, renderKpNode, cwProgress],
  );
