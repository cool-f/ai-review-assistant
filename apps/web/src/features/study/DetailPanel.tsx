...
import type { KnowledgePoint, Homework, HomeworkDetail, GeneratedQuestion, ProgressStatus, StudyProgress } from "@/shared/types";
...
export interface DetailPanelProps {
  ...
  onHomeworkRefresh?: () => Promise<void> | void;
  onKnowledgePointUpdated?: (knowledgePoint: KnowledgePoint) => void;
  onKnowledgePointOpen?: (knowledgePointId: string) => void;
}
...
export default function DetailPanel({
  ...
  const [editingKnowledgePoint, setEditingKnowledgePoint] = useState(false);
  const [editTitle, setEditTitle] = useState("");
  const [editContent, setEditContent] = useState("");
  const [editPage, setEditPage] = useState("");
  const [editError, setEditError] = useState<string | null>(null);
  const [relatedKnowledge, setRelatedKnowledge] = useState<Array<{
    id: string; similarity: number; linked_kp: {
      id: string; title: string; courseware_id: string; courseware_title: string;
    };
  }>>([]);

  useEffect(() => {
    setEditingKnowledgePoint(false);
    setEditError(null);
    setEditTitle(knowledgePoint?.title ?? "");
    setEditContent(knowledgePoint?.content ?? "");
    setEditPage(knowledgePoint?.page_number?.toString() ?? "");
    if (!knowledgePoint) {
      setRelatedKnowledge([]);
      return;
    }
    if (knowledgePoint.indexing_status === "processing") {
      setRelatedKnowledge([]);
      return;
    }
    let cancelled = false;
    client.get<{ links: typeof relatedKnowledge }>(`/knowledge-points/${knowledgePoint.id}/links`, {
      params: { course_id: courseId },
    }).then((response) => {
      if (!cancelled) {
        setRelatedKnowledge(
          response.data.links.filter((link) => link.linked_kp.courseware_id !== knowledgePoint.courseware_id),
        );
      }
    }).catch(() => {
      if (!cancelled) setRelatedKnowledge([]);
    });
    return () => { cancelled = true; };
  }, [courseId, knowledgePoint?.id, knowledgePoint?.indexing_status, knowledgePoint?.revision]); // eslint-disable-line react-hooks/exhaustive-deps

  useEffect(() => {
    if (!knowledgePoint || knowledgePoint.indexing_status !== "processing") return;
    let cancelled = false;
    ...
    const pollIndexingStatus = async () => {
      try {
        const response = await client.get<KnowledgePoint>(
          `/knowledge-points/${knowledgePoint.id}`,
          { params: { course_id: courseId } },
        );
        if (cancelled) return;
        onKnowledgePointUpdated?.(response.data);
        if (response.data.indexing_status === "processing") {
          timer = window.setTimeout(() => void pollIndexingStatus(), 1200);
        }
      } catch {
        if (!cancelled) {
          timer = window.setTimeout(() => void pollIndexingStatus(), 2000);
        }
      }
    };

    timer = window.setTimeout(() => void pollIndexingStatus(), 800);
    return () => {
      cancelled = true;
      if (timer !== undefined) window.clearTimeout(timer);
    };
  }, [courseId, knowledgePoint?.id, knowledgePoint?.indexing_status, onKnowledgePointUpdated]);

  const handleSaveKnowledgePoint = useCallback(async () => {
    if (!knowledgePoint || !editTitle.trim() || !editContent.trim()) return;
    setEditError(null);
    try {
      const response = await client.patch<KnowledgePoint>(
        `/knowledge-points/${knowledgePoint.id}`,
        {
          title: editTitle.trim(),
          content: editContent.trim(),
          page_number: editPage.trim() ? Number(editPage) : null,
        },
        { params: { course_id: courseId } },
      );
      onKnowledgePointUpdated?.(response.data);
  ...
  }, [courseId, editContent, editPage, editTitle, knowledgePoint, onKnowledgePointUpdated]);
  const [solvingHomework, setSolvingHomework] = useState(false);
  const [solveError, setSolveError] = useState<string | null>(null);
  const [liveAnswers, setLiveAnswers] = useState<Record<number, string>>({});

  useEffect(() => {
    setSolvingHomework(false);
    setSolveError(null);
    setLiveAnswers({});
  }, [homework?.id]);

  const handleSolveHomework = useCallback(async () => {
    if (!homework || solvingHomework) return;
    setSolvingHomework(true);
    setSolveError(null);
    // Homework jobs replay their event history on reconnect, so rebuild the
    // transient answer view from an empty snapshot to avoid duplicate tokens.
    setLiveAnswers({});
    try {
      const response = await fetch(
        `/api/homeworks/${homework.id}/solve?course_id=${encodeURIComponent(courseId)}`,
        { method: "POST", headers: { "Content-Type": "application/json" } },
      );
      if (!response.ok || !response.body) {
        const detail = await response.text();
        throw new Error(detail || `解题请求失败 (${response.status})`);
      }
      ...
      while (true) {
        ...
        for (const raw of events) {
          ...
          const event = JSON.parse(line.slice(6)) as {
            type: string; question_number?: number; content?: string; message?: string;
          };
          if (event.type === "token" && event.question_number != null) {
            setLiveAnswers((current) => ({
              ...current,
              [event.question_number!]: (current[event.question_number!] ?? "") + (event.content ?? ""),
            }));
          } else if (event.type === "question_error") {
            setSolveError(event.message ?? "部分题目解答失败，可重试");
          }
        }
      }
      await onHomeworkRefresh?.();
    } catch (error) {
      setSolveError(error instanceof Error ? error.message : "解题失败，请重试");
    } finally {
      setSolvingHomework(false);
    }
  }, [courseId, homework, onHomeworkRefresh, solvingHomework]);
    ...
    const fetchProgress = async () => {
      ...
      try {
        ...
        if (!cancelled) {
          ...
          setProgressManual(res.data.manual_status != null);
        }
      } catch {
        if (!cancelled) {
  ...
  }, [courseId, knowledgePoint?.id]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const handleMarkStatus = useCallback(
    ...
    [courseId, knowledgePoint, onProgressChange],
  );

  const handleClearManualStatus = useCallback(async () => {
    if (!knowledgePoint) return;
    setProgressLoading(true);
    try {
      const res = await client.patch<StudyProgress>(
        `/knowledge-points/${knowledgePoint.id}/progress`,
        { action: "clear_manual_status" },
        { params: { course_id: courseId } },
      );
      setProgressStatus(res.data.status as ProgressStatus);
      setProgressManual(false);
  ...
  const handleGenerateQuestions = async (kpId: string) => {
    ...
    try {
      ...
      const url = `${baseURL}/knowledge-points/${kpId}/generate-questions?course_id=${encodeURIComponent(courseId)}`;
          ...
          <div className="space-y-4 p-4">
            ...
            <div className="flex items-center gap-2 text-xs text-gray-500">
              <span className={`inline-block rounded-full px-2 py-px text-[10px] font-medium ${
                ...
                homework.status === "partial" ? "bg-orange-100 text-orange-700" :
                "bg-gray-100 text-gray-500"
              }`}>
                {homework.status === "completed" ? "已完成" :
                 homework.status === "processing" ? "处理中" :
                 homework.status === "failed" ? "失败" :
                 homework.status === "partial" ? "部分完成" :
                 homework.status === "ready" ? "待解答" : homework.status}
              </span>
              <span>{homework.file_type.toUpperCase()}</span>
              <span>{(homework.file_size / 1024).toFixed(0)} KB</span>
            </div>

            {(homework as HomeworkDetail).solutions?.some((solution) => !solution.answer_text) && (
              <button
                type="button"
                onClick={() => void handleSolveHomework()}
                disabled={solvingHomework}
              ...
              >
                {solvingHomework ? "正在逐题解答…" : homework.status === "partial" || homework.status === "failed" ? "继续解题" : "开始解题"}
              </button>
            )}
            {solveError && <p className="rounded bg-red-50 px-3 py-2 text-xs text-red-600">{solveError}</p>}
                  ...
                  (s) => (
                    ...
                    >
                      <div className="flex items-center gap-2 text-xs text-gray-500">
                        ...
                        {(s.answer_text || liveAnswers[s.question_number]) ? (
                      ...
                      {(liveAnswers[s.question_number] || s.answer_text) && (
                      ...
                      {s.knowledge_point_links.length > 0 && (
                        <div className="mt-2 flex flex-wrap gap-1">
                          {s.knowledge_point_links.map((link) => (
                            <span
                              key={link.id}
                              className="rounded-full bg-blue-50 px-2 py-1 text-[10px] text-blue-700"
                              title={`${link.match_method} · ${(link.relevance_score * 100).toFixed(0)}%`}
                            >
                              {link.knowledge_point_title}
                            </span>
                          ))}
                        </div>
                      )}
          ...
          <div className="space-y-4 p-4">
            ...
            <div>
              ...
              {progressManual && (
                <button
                  type="button"
                  onClick={() => void handleClearManualStatus()}
                  disabled={progressLoading}
            ...
            <div className="flex items-start justify-between gap-3">
              <div>
                <h2 className="text-lg font-semibold text-gray-800">{knowledgePoint.title}</h2>
                <p className="mt-1 text-[11px] text-gray-400">
                  修订版 {knowledgePoint.revision} · 索引{
                    knowledgePoint.indexing_status === "processing" ? "更新中" :
                    knowledgePoint.indexing_status === "failed" ? "失败" : "已同步"
                  }
                </p>
              </div>
              <button
                type="button"
                onClick={() => setEditingKnowledgePoint((current) => !current)}
                className="rounded border border-gray-200 px-2 py-1 text-xs text-gray-600 hover:bg-gray-50"
              >
                {editingKnowledgePoint ? "取消编辑" : "纠错编辑"}
              </button>
            </div>
            {knowledgePoint.indexing_error && (
              <p className="rounded bg-orange-50 px-3 py-2 text-xs text-orange-700">语义索引降级：{knowledgePoint.indexing_error}</p>
            )}
            ...
            {editingKnowledgePoint ? (
              <div className="space-y-2 rounded-lg border border-blue-200 bg-blue-50 p-3">
                <input value={editTitle} onChange={(event) => setEditTitle(event.target.value)} className="w-full rounded border border-gray-300 px-2 py-1.5 text-sm" aria-label="知识点标题" />
                <textarea value={editContent} onChange={(event) => setEditContent(event.target.value)} rows={8} className="w-full rounded border border-gray-300 px-2 py-1.5 text-sm" aria-label="知识点内容" />
                <input value={editPage} onChange={(event) => setEditPage(event.target.value)} type="number" min="1" placeholder="来源页码" className="w-full rounded border border-gray-300 px-2 py-1.5 text-sm" aria-label="来源页码" />
                {editError && <p className="text-xs text-red-600">{editError}</p>}
                <button type="button" onClick={() => void handleSaveKnowledgePoint()} className="rounded bg-blue-600 px-3 py-1.5 text-xs font-medium text-white">保存并重新索引</button>
              </div>
            ) : (
            ...
            {relatedKnowledge.length > 0 && (
              <div>
                <h3 className="mb-2 text-sm font-medium text-gray-700">跨课件关联</h3>
                <div className="space-y-1.5">
                  {relatedKnowledge.map((link) => (
                    <button
                      key={link.id}
                      type="button"
                      onClick={() => onKnowledgePointOpen?.(link.linked_kp.id)}
                    ...
                    >
                      <span><span className="text-gray-400">{link.linked_kp.courseware_title} / </span>{link.linked_kp.title}</span>
                      <span className="text-gray-400">{Math.round(link.similarity * 100)}%</span>
                    </button>
                  ))}
                </div>
              </div>
