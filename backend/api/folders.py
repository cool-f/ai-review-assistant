...
from backend.models import Folder, Courseware, Homework
...
async def create_folder(
    ...
    """创建新文件夹"""
    folder = Folder(name=body.name.strip(), category=body.category)
    db.add(folder)
    await db.commit()
...
async def list_folders(
    ...
    result = await db.execute(
        select(Folder)
        .where(Folder.category == category)
...
async def move_item(
    ...
    item_result = await db.execute(
        select(model).where(model.id == body.item_id)
    )
