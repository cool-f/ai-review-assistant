...
from backend.models import KnowledgePoint, KnowledgePointLink, Courseware
from backend.schemas.link import (
...
async def get_knowledge_point_links(
    ...
    if linked_kp_ids:
        kp_info_result = await db.execute(
            select(KnowledgePoint.id, KnowledgePoint.title, KnowledgePoint.courseware_id)
            .where(KnowledgePoint.id.in_(list(linked_kp_ids)))
        )
